...
// The texturing
void Wolf3D::addCube(std::vector<glm::vec3>& vertexPositions, std::vector<glm::vec4>& textureCoordinates, int x, int z,
                     int type)
{
	// todo implement this
	...
	glm::vec2 min = vec2(tileSizeWithBorder.x * 2 * (type % 8), row * tileSizeWithBorder.y) / textureSize;
	...
	min = vec2(tileSizeWithBorder.x * (2 * ((type) % 8) + 1), row * tileSizeWithBorder.y) / textureSize;
